...
*/
    func OFFPricesAuth(username: String, password: String, completion: @escaping (_ postResult: Result<OFFPricesRequired.Auth, OFFPricesError>) -> Void) {
        fetch(request: OFFPricesRequest(username: username, password: password, needCookie: true), responses: [200:OFFPricesRequired.Auth.self]) { (result) in
            completion(result)
            return
        }
    }
...
*/
    convenience init(username: String, password: String, needCookie: Bool) {
        self.init(api: .auth)
